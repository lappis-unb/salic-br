...
class Prorrogacao extends MinC_Db_Table_Abstract {
    ...
    public function buscar($where=array(), $order=array(), $tamanho=-1, $inicio=-1) {
        ...
        $slctQtdeMeses = "(SELECT DATEDIFF(MONTH,(SELECT max(DtRecibo) FROM SAC.dbo.Captacao WHERE Anoprojeto = pr.AnoProjeto and Sequencial = pr.Sequencial), {$this->getDate()}) as qtdeMeses WHERE (SELECT max(DtRecibo) FROM SAC.dbo.Captacao WHERE Anoprojeto = pr.AnoProjeto and Sequencial = pr.Sequencial)<>'1900-01-01 00:00:00.000')";
        $slct->from(
                array('pr'=>$this->_name),
